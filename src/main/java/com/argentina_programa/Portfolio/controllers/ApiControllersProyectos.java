...
public class ApiControllersProyectos {
    ...
    public ResponseEntity <Object> save (@RequestBody Proyectos estudio, @RequestHeader("Authorization") String token){
    ...
    public ResponseEntity <Object> update ( @RequestBody Proyectos estudio,@PathVariable int id, @RequestHeader("Authorization") String token){
        boolean bandera = getAuthentication(token);
        if(bandera)
